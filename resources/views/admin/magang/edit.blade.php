        ...
        <form method="POST" action="{{ route('admin.magang.update', $magang->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-grid-responsive mb-4">
                <div class="form-field-wrapper">
                    ...
                    <input class="profileInput" type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $displayName) }}" required placeholder="Contoh: Ahmad Fauzi Pratama" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">NIK / No. Identitas Sistem <span class="text-danger">*</span></label>
                    <input class="profileInput" type="text" name="nik" value="{{ old('nik', $magang->nik) }}" required placeholder="Contoh: MG202601" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">Email Login <span class="text-danger">*</span></label>
                    <input class="profileInput" type="email" name="email" value="{{ old('email', $magang->email) }}" required placeholder="Contoh: ahmad@kampus.ac.id" />
                </div>
                ...
                <div class="form-field-wrapper">
                    ...
                    <input class="profileInput" type="text" name="no_hp" value="{{ old('no_hp', $magang->no_hp) }}" placeholder="Contoh: 081234567890" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">Status Akun &amp; Penugasan <span class="text-danger">*</span></label>
                    <select name="is_active" class="filterSelect">
                        <option value="1" {{ old('is_active', $magang->is_active) == 1 ? 'selected' : '' }}>Aktif Berjalan</option>
                        <option value="0" {{ old('is_active', $magang->is_active) == 0 ? 'selected' : '' }}>Selesai / Nonaktif</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-grid-responsive mb-4">
                <div class="form-field-wrapper">
                    ...
                    <input class="profileInput" type="text" name="asal_instansi" value="{{ old('asal_instansi', $detail?->asal_instansi) }}" required placeholder="Contoh: Universitas Negeri Yogyakarta" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">NIM / NISN</label>
                    <input class="profileInput" type="text" name="nim_nisn" value="{{ old('nim_nisn', $detail?->nim_nisn) }}" placeholder="Contoh: 210101002" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">Program Studi / Jurusan</label>
                    <input class="profileInput" type="text" name="jurusan" value="{{ old('jurusan', $detail?->jurusan) }}" placeholder="Contoh: Pendidikan Luar Sekolah" />
                </div>

                <div class="form-field-wrapper">
                    <label class="filterFieldLabel">Dosen / Guru Pembimbing</label>
                    <input class="profileInput" type="text" name="pembimbing_lapangan" value="{{ old('pembimbing_lapangan', $detail?->pembimbing_lapangan) }}" placeholder="Contoh: Dr. Hendra Santoso, M.Pd" />
                </div>
