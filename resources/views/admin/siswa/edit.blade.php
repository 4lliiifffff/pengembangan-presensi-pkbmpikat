    ...
    <div class="laporanHeaderCard">
        <div class="laporanHeaderMain">
            <div class="laporanTitleBlock">
                ...
                <h1 class="laporanMainTitle">Edit Data Siswa</h1>
                <p class="laporanDesc">Perbarui rombongan belajar, status siklus, dan data wali murid</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.siswa.update', $siswa) }}">
                    ...
                    <select class="filterSelect" name="tutor_id">
                        ...
                        @if (isset($tutors))
                            @foreach ($tutors as $tutor)
                                <option value="{{ $tutor->id }}" {{ old('tutor_id', $siswa->tutor_id) == $tutor->id ? 'selected' : '' }}>
            ...
            <div style="margin-top:20px;">
                <label style="display:flex;align-items:center;gap:12px;cursor:pointer;font-size:13.5px;font-weight:700;color:var(--text);padding:12px 16px;background:var(--card-alt,#f8fafc);border-radius:10px;border:1px solid var(--border,#e2e8f0);">
                    <input type="checkbox" name="is_abk" value="1" {{ old('is_abk', $siswa->is_abk) ? 'checked' : '' }} style="width:18px;height:18px;">
