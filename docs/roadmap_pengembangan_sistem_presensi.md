...
* 🟢 **Refactoring Seluruh View Kosong (@empty):** [SELESAI] Mengeliminasi ikon ad-hoc tanpa class (seperti `icon-2xl opacity-40` di `admin/laporan` & `kepsek/presensi`), menambahkan badge ikon berkode warna pada seluruh tabel yang sebelumnya tanpa ikon (`admin/karyawan`, `admin/siswa`, `admin/jadwal_rutin`, `admin/jenjang_paket`, `admin/kategori_tutorial`, `admin/kelas`, `admin/izin`), serta memetakan class warisan (`empty-state-standard`, `empty-icon`, `empty-title`, `empty-desc`) di modul Tutor, Siswa, Magang, dan Kepsek ke token desain terpadu.
