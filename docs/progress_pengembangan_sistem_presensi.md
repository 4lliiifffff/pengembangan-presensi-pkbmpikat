...
| 23 | **Standardisasi Arsitektur Upload, Storage Disk & Eliminasi Redundansi** | Storage & File System | Menghapus prefix `'uploads/'` di seluruh controller, konsolidasi direktori `storage/app/public/uploads/*` ke root disk publik (`foto_karyawan/`, `presensi/`, `presensi_karyawan/`, `presensi_siswa/`, `profiles/`), hapus folder redundan `uploads/`, sambung ulang symlink `public/storage`, standarisasi Eloquent Accessors (`foto_mulai_url`, `foto_selesai_url`, `foto_url`), perbarui seeder `RealDataSeeder`, dan ganti pemanggilan raw `asset()` di Blade. | 🟢 Selesai |
| 24 | **Standardisasi Persona Akun Demo Seeder & Antarmuka Cepat (Quick Login)** | Testing & Demo Data | Menyelaraskan nama 5 persona utama: Admin (Kak Tasya), Kepala Sekolah (Bu Dara), Tutor (Kak Tari), Mahasiswa Magang (Alif), dan Murid/Siswa (Zeldi) pada seluruh seeders (`UserRoleSeeder`, `AdminSeeder`, `MagangSeeder`, `SiswaUserSeeder`, `SiswaSeeder`), kartu login cepat (`auth/login`), floating dropdown *Quick Role Switcher*, controller `AuthWebController`, placeholder form admin, dan unit test. | 🟢 Selesai |
| 25 | **Kebijakan Bebas Radius Presensi untuk Role Admin & Kepala Sekolah** | Geofencing & Policy | Menonaktifkan batasan radius presensi geofence khusus untuk peran Admin (`admin`) dan Kepala Sekolah (`kepala_sekolah`) agar fleksibel melakukan absensi saat bertugas dinas luar, rapat koordinasi, atau ada keperluan mendesak. Koordinat GPS riil tetap tercatat, feedback peta Leaflet menampilkan status `Bebas Radius Aktif (Tugas Dinas/Rapat)` dengan radar dot `pulse-blue`, banner edukasi role, dan 100% lulus pengujian otomatis `GeofencingTest`. | 🟢 Selesai |
| 26 | **Perbaikan Pelacakan Lokasi Presensi Pulang & Robust Geolocation Fallback** | Geofencing & Bugfix | Mengatasi kegagalan pelacakan lokasi saat presensi pulang akibat koordinat target undefined (dropdown lokasi tidak ada saat checkout), menghilangkan uncaught exception di Leaflet, menghapus class `.d-none` saat inisialisasi peta, auto-trigger `invalidateSize()`, serta memperbaiki deteksi moda tutor (`isSekolahModa`) agar sesi daring/kunjungan tidak salah memicu peringatan geofence. | 🟢 Selesai |
| 27 | **Harmonisasi Styling Status Siap Pulang & Desain Ramah Mata (Mobile)** | UI/UX & Responsive | Menghilangkan duplikasi banner bertumpuk dengan warna kontras keras (`text-blue-900`, `bg-blue-50`, oranye vs biru), menggantikannya dengan satu kartu ringkas terpadu `.card.border-base.bg-card-alt.rounded-2xl`, badge lembut `.badge.bg-success-light.text-success`, tipografi berbasis token CSS `var(--text)` / `var(--muted)`, serta tata letak flexbox responsif tanpa *horizontal overflow* pada ponsel. | 🟢 Selesai |
| 28 | **Penyelarasan Styling Tema Banner Akses Fleksibel & Binding Dark Mode Tailwind v4** | CSS & Theme Bugfix | Menambahkan direktif `@custom-variant dark (&:where([data-theme=dark], [data-theme=dark] *));` pada `resources/css/app.css` agar Tailwind CSS v4 tidak menerapkan class `dark:*` berdasarkan preferensi OS (`prefers-color-scheme: dark`) saat web aplikasi berada pada tema terang (`data-theme="light"`). Menstandarisasi banner *Akses Fleksibel (Bebas Radius) Aktif* menggunakan class `.statusBanner.flexible` dan badge `.badge-flexible` dengan variabel warna `var(--text)`, `var(--muted)`, dan `var(--card)` sehingga 100% harmonis dan kontras optimal pada tema terang maupun tema gelap. | 🟢 Selesai |
| 29 | **Optimalisasi Layout Spacing & Responsivitas Mobile Dashboard Admin** | UI/UX & Responsive | Memperbaiki bug penutupan tag pembungkus `.laporanHeader` yang memicu *nested double-padding* pada mobile, menyelaraskan kartu ringkasan KPI (`.summaryCard` & `.summarySub`), menambahkan pintasan menu cepat operasional Admin (`.adminQuickSection` & `.adminQuickGrid`), memodernisasi grafik mingguan dengan indikator angka real-time (`.barChart`, `.barCount`), serta menerapkan layout adaptif ergonomis pada kartu aktivitas presensi (`.activityRow`) di layar $\le 540$px. | 🟢 Selesai |
| 30 | **Standardisasi Desain Empty State Tabel & Kartu Mobile (Design System 5 Role)** | UI/UX & Responsive | Harmonisasi gaya visual *empty state* dengan mengadopsi wadah pill badge berkontur lembut (`.tableEmptyIconWrap`, 52px, border-radius 16px, background aksen transparan, tema terang & gelap) menggantikan ikon polos 44px (`.tableEmptyIcon`), menghadirkan komponen `.tableEmptyCard` untuk mobile card list, serta refactoring menyeluruh pada seluruh view tabel/kartu kosong di 5 peran pengguna: **Admin** (`laporan`, `karyawan`, `siswa`, `jadwal_rutin`, `jadwal_kerja`, `lokasi_presensi`, `magang`, `jenjang_paket`, `kategori_tutorial`, `kelas`, `izin`, `payroll`), **Kepala Sekolah** (`presensi`, `pengajuan_izin`, `lupa_lapor`, `laporan`, `dashboard`), **Tutor** (`riwayat`, `payroll`, `dashboard`), **Siswa** (`riwayat`, `dashboard`), dan **Magang** (`riwayat`, `dashboard`) agar 100% konsisten, modern, dan responsif. | 🟢 Selesai |
