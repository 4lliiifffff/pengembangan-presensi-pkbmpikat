...
| 30 | **Standardisasi Desain Empty State Tabel & Kartu Mobile (Design System)** | UI/UX & Responsive | Harmonisasi gaya visual *empty state* dengan mengadopsi wadah pill badge berkontur lembut (`.tableEmptyIconWrap`, 52px, border-radius 16px, background aksen transparan, tema terang & gelap) menggantikan ikon polos 44px (`.tableEmptyIcon`), menghadirkan komponen `.tableEmptyCard` untuk mobile card list, serta refactoring menyeluruh pada seluruh view tabel/kartu kosong (`admin/laporan`, `admin/karyawan`, `admin/siswa`, `admin/jadwal_rutin`, `admin/jadwal_kerja`, `admin/lokasi_presensi`, `admin/magang`, `admin/jenjang_paket`, `admin/kategori_tutorial`, `admin/kelas`, `admin/izin`, `kepsek/presensi`, `kepsek/pengajuan_izin`, `kepsek/lupa_lapor`, serta riwayat presensi tutor/siswa/magang) agar 100% konsisten, modern, dan responsif. | 🟢 Selesai |
